        ...
        <div className={`fixed inset-0 ${isDark ? "bg-black/60" : "bg-[#1A1A1A]/40"} flex justify-center items-center z-50 p-4 overflow-y-auto`}>
          <div className={`${cardBg} border-4 ${b} w-full max-w-lg shadow-[8px_8px_0px_0px] ${shadow} my-auto`}>
            ...
            <div className={`p-4 border-b-4 ${b} bg-[#FFC700] text-black flex justify-between items-center`}>
              ...
              <button onClick={() => setIsModalOpen(false)} className={`w-8 h-8 border-2 ${b} bg-white text-black flex items-center justify-center font-black text-lg hover:bg-[#FF6B6B] transition-colors focus:outline-none`}>
                &times;
              </button>
            </div>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div>
                ...
                <select
                  ...
                  className={`w-full border-2 ${b} p-2.5 text-xs font-bold uppercase tracking-wide focus:outline-none appearance-none rounded-none ${inputBg}`}
                >
                  <option value="" className={isDark ? "text-gray-400" : "text-black"}>
                    Pilih Kategori
                  </option>
                  {kategori.map((k) => (
                    <option key={k.id_kategori} value={k.id_kategori} className={isDark ? "text-white" : "text-black"}>
                      {k.nama_kategori}
                    </option>
                  ))}
                </select>
              ...
              <div className={`border-2 border-dashed ${isDark ? "border-white/40" : "border-[#1A1A1A]/40"} p-4 ${inputBg} space-y-3`}>
                <div>
                  <label className="block text-[10px] font-black uppercase tracking-wider mb-1">Unggah Berkas Gambar</label>
                  <input
                    type="file"
                    accept="image/*"
                ...
                <div>
                  <label className="block text-[10px] font-black uppercase tracking-wider mb-1">Atau Gunakan Tautan URL</label>
                ...
                {formData.gambar && (
                  <div className="pt-2">
                    <p className="text-[9px] font-black uppercase opacity-50 mb-1">Pratinjau Gambar:</p>
