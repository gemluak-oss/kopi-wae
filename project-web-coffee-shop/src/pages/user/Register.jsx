...
export default function Register() {
  ...
  const [formData, setFormData] = useState({ user_name: "", email: "", password: "", no_hp: "" });
  ...
  return (
    <>
      ...
      <main className="min-h-screen bg-[#E8D8C6] flex items-center justify-center px-4 py-12 font-body relative overflow-hidden">
        ...
        {/* Background decorations - ringkas */}
        ...
        <div className="w-full max-w-5xl grid grid-cols-1 lg:grid-cols-12 rounded-2xl overflow-hidden shadow-2xl shadow-[#C77A23]/20 border border-[#C77A23]/20 relative z-10">
          ...
          <section className="lg:col-span-5 bg-[#C77A23] p-8 lg:p-10 flex flex-col justify-between text-white relative overflow-hidden">
            ...
            <div className="relative z-10">
              ...
              <h2 className="font-serif text-3xl lg:text-4xl font-bold leading-tight mb-4 text-center">Bergabung Bersama Kami</h2>
              <p className="text-sm font-body font-light leading-relaxed opacity-90 text-center max-w-xs mx-auto">Daftar sekarang untuk menyimpan menu favorit dan nikmati layanan terbaik Kopi Wae.</p>
            </div>
          ...
          <section className="lg:col-span-7 p-8 lg:p-12 bg-white flex flex-col justify-center">
            <div className="flex items-center justify-between mb-8">
              <div>
                <h2 className="font-serif text-3xl font-bold text-[#3A2F2B]">Daftar Akun</h2>
                <p className="text-sm text-[#3A2F2B]/50 font-body mt-1">Lengkapi data di bawah ini untuk membuat akun baru.</p>
              </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-5" autoComplete="off">
              {[
                { label: "Nama Lengkap", name: "user_name", type: "text", placeholder: "Jonathan Doe", icon: "M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" },
                { label: "Alamat Email", name: "email", type: "email", placeholder: "coffee@email.com", icon: "M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" },
                {
                ...
                <div key={f.name}>
                  <label className="block text-xs font-body font-bold uppercase tracking-wider text-[#3A2F2B] mb-2">{f.label}</label>
              ...
              <button
                type="submit"
                disabled={loadingRegister}
                className={`w-full py-4 rounded-xl font-body font-bold text-sm uppercase tracking-wider transition-all flex items-center justify-center gap-2 ${loadingRegister ? "bg-gray-300 text-gray-500 cursor-not-allowed" : "bg-[#C77A23] text-white hover:bg-[#3A2F2B] shadow-lg shadow-[#C77A23]/20 hover:shadow-xl hover:shadow-[#3A2F2B]/20"}`}
              >
                {loadingRegister ? (
                  <>
            ...
            <p className="text-center text-sm font-body text-[#3A2F2B]/60 mt-8">
              Sudah punya akun?{" "}
              <button type="button" onClick={() => navigate("/login")} className="text-[#C77A23] font-bold hover:text-[#3A2F2B] transition-colors">
                Masuk di sini
              </button>
            </p>
          </section>
